...
public class PrintSimplePanel extends SimplePanel {
	...
	public PrintSimplePanel(Student student,
			Collection<ProgramLanguage> newLanguage2) {
	...
	public PrintSimplePanel(CV cv) {
		educations = cv.getEducations();
		languages = cv.getLanguages();
		programLanguage = cv.getProgramLanguages();
		newLanguage = new ArrayList<ProgramLanguage>();
		sertificates = cv.getSertificates();
		workExps = cv.getWorkExps();
		qualities = cv.getQualities();
		others = cv.getOthers();
	...
	public void onModuleLoad() {
		...
		// vLayout2.addMember(new BlueBox((Integer) null, "50%", "")); по идее
		// так будет работать
		if (student != null) {
			vLayout2.addMember(new BlueBox((Integer) null, "25%", student
					.getAddress()));
			vLayout2.addMember(new BlueBox((Integer) null, "25%", student
					.getSkype() == null ? "" : student.getSkype()));
			vLayout2.addMember(new BlueBox((Integer) null, "25%", student
					.getPhone()));
			vLayout2.addMember(new BlueBox((Integer) null, "25%", student
					.getEmail()));
			...
			String patr = student.getPatronymic() == null ? "" : student
					.getPatronymic();
			BlueBox FIO = new BlueBox("30%", "90", "<h2 align=\"center\">"
		...
		vLayout.addMember(new BlueBox((String) null, "30",
				"Желаемая должность", true));
		...
		if (workExps != null && workExps.size() != 0) {
			...
			for (WorkExp I : workExps) {
				...
				if (I.getIsNow()) {
					...
					hLayout2.addMember(new BlueBox("20%", null, "С "
							+ I.getStartYear() + " " + duration));
					...
					hLayout2.addMember(new BlueBox("20%", null, "С "
							+ I.getStartYear() + "<br>На протяжении "
							+ I.getDuration() + " " + duration));
				}
				hLayout2.addMember(new BlueBox("10px", null, ""));
				hLayout2.addMember(new BlueBox("*", null, I.getRole() + " "
						+ I.getNameOfInstitution()));
			}
		}
		if (educations != null && educations.size() != 0) {
			vLayout.addMember(new BlueBox((String) null, "30", "Образование",
					true));
			...
			for (Education I : educations) {
				...
				hLayout3.addMember(new BlueBox("20%", null, I.getStartYear()
						+ " - " + I.getEndYear()));
				hLayout3.addMember(new BlueBox("10px", null, ""));
				hLayout3.addMember(new BlueBox("*", null, "Специальность "
						+ I.getSpecialty() + ", Факультет " + I.getFaculty()
						+ "; " + I.getNameOfInstitution()));
			}
		}
		if ((programLanguage != null && programLanguage.size() != 0)
				|| (newLanguage != null && newLanguage.size() != 0)) {
			vLayout.addMember(new BlueBox((String) null, "30",
					"Профессиональные навыки", true));
			...
			if (newLanguage.size() != 0) {
				for (ProgramLanguage k : newLanguage) {
					if (k.getLevel() == 2)
						level1 += "<a class=\"green\">" + k.getTitle()
								+ ", </a>";
					if (k.getLevel() == 1)
						level2 += "<a class=\"green\">" + k.getTitle()
								+ ", </a>";
					if (k.getLevel() == 0)
						level3 += "<a class=\"green\">" + k.getTitle()
								+ ", </a>";
				}
				// level3 = level3.substring(0,level3.length()-1);

			}
			if (level1 == "Владение технологиями на продвинутом уровне<br>")
				level1 = "";
			if (level2 == "Владение технологиями на среднем уровне<br>")
				level2 = "";
			if (level3 == "Владение технологиями на уровне начинающего<br>")
				level3 = " ";
		...
		/*
		 * HLayout hLayout5 = new HLayout(); vLayout.addMember(hLayout5);
		 * hLayout5.setHeight100(); hLayout5.addMember(new BlueBox("20%", null,
		 * "")); hLayout5.addMember(new BlueBox("20%", null, "Технологии"));
		 * hLayout5.addMember(new BlueBox("*", null,
		 * "GWT, Win32 API, WinForms")); HLayout hLayout6 = new HLayout();
		 * vLayout.addMember(hLayout6); hLayout6.setHeight100();
		 ...
		 * HLayout hLayout7 = new HLayout(); vLayout.addMember(hLayout7);
		 * hLayout7.setHeight100(); hLayout7.addMember(new BlueBox("20%", null,
		 * "")); hLayout7.addMember(new BlueBox("20%", null, "Базы данных"));
		 * hLayout7.addMember(new BlueBox("*", null,
		 * "SQL Server, MySQL, TSQL")); HLayout hLayout8 = new HLayout();
		 * vLayout.addMember(hLayout8); hLayout8.setHeight100();
		 * hLayout8.addMember(new BlueBox("20%", null, ""));
		 * hLayout8.addMember(new BlueBox("20%", null, "Офисные программы"));
		 * hLayout8.addMember(new BlueBox("*", null,
		 * "MS Word, MS Power Point, MS Acces, Open Office")); HLayout hLayout9
		 * = new HLayout();
		 * 
		 * vLayout.addMember(hLayout9); hLayout9.setHeight100();
		 * hLayout9.addMember(new BlueBox("20%", null, ""));
		 * hLayout9.addMember(new BlueBox("20%", null,
		 * "Графические программы")); hLayout9.addMember(new BlueBox("*", null,
		 * "Photoshop"));
		 */
		if (sertificates != null && sertificates.size() != 0) {
			vLayout.addMember(new BlueBox((String) null, "30", "Сертификаты",
					true));
			...
			for (Sertificate I : sertificates) {
				...
				hLayout10.addMember(new BlueBox("20%", null, ""
						+ I.getSertificateYear()));
				hLayout10.addMember(new BlueBox("10px", null, ""));

				hLayout10.addMember(new BlueBox("*", null, I
						.getSertificateName()));
			}
		}

		if (languages != null && languages.size() != 0) {
			vLayout.addMember(new BlueBox((String) null, "30", "Знания языков",
					true));
			...
			for (Language I : languages) {
				...
				hLayout11.addMember(new BlueBox("*", null, I.getTitle() + " - "
						+ level));
